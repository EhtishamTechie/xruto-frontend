...
const LoginScreen = ({ onLogin }) => {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-xr-bg px-4 py-10 md:px-6 transition-colors duration-300">
      <div aria-hidden className="pointer-events-none fixed inset-0 bg-grid opacity-[0.25] dark:opacity-[0.35]" />
      <div aria-hidden className="pointer-events-none fixed inset-0 bg-noise" />
      
      {/* Theme Toggle */}
      <button
        onClick={() => setIsDark(!isDark)}
        className="absolute top-6 right-6 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-xr-elevated text-xr-muted border border-xr-border shadow-sm transition hover:text-xr-brand"
        aria-label="Toggle theme"
      >
        <Ico d={isDark ? ICONS.sun : ICONS.moon} className="w-5 h-5" />
      </button>

      <UiCard variant="glass" className="relative z-10 w-full max-w-sm p-6 shadow-panel md:max-w-md md:p-10 transition-colors duration-300">
        <div className="flex flex-col items-center">
          <div className="mb-8 w-full text-center">
            <div className="inline-flex flex-col items-center">
              <div className="mb-3 flex h-14 w-14 items-center justify-center overflow-hidden rounded-card border border-xr-border bg-xr-bg shadow-lg">
                <BrandLogo className="h-14 w-14 text-xr-text" alt="xRuto" />
              </div>
              <h1 className="text-2xl font-bold tracking-tight text-xr-text sm:text-3xl">xRuto</h1>
              <p className="mt-1 text-[11px] font-medium uppercase tracking-[0.28em] text-xr-muted">Logistics Intelligence</p>
            </div>
          </div>
          <h2 className="mb-6 text-center text-lg font-semibold text-xr-text">Sign in to your account</h2>
          {error && (
            <div className="mb-4 w-full rounded-control border border-xr-danger/25 bg-xr-danger/10 px-4 py-3 text-center text-sm text-red-500">{error}</div>
          )}
...
const App = () => {
  ...
  useEffect(() => {
    // Sync theme on app load for authenticated views
    const savedTheme = localStorage.getItem('xruto_theme');
